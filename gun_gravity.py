...
def check_is_bareer_for_transporting_present():
	...
	Барьер - предмет со свойством "ground". (v4)
	'''

	point_for_transporting = scene.objects['point_for_transporting']  # должна присутствовать
	barrier = crosshair.rayCast(point_for_transporting, crosshair, 0, 'ground', 0, 1, 0, 65535)  # 65535 - числ. предст. групп - см. краткий конспект Blender
	#print(barrier)
	return barrier[0]
...
def check_is_bareer_for_throwing_present():
	...
	barrier = crosshair.rayCastTo((crosshairs_world_position_changed), 0.0, 'ground')
	#print(barrier)
	return barrier
